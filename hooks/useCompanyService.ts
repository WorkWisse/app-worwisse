...
export const useCompanyService = (): UseCompanyServiceReturn => {
  ...
  const getCompanies = async (
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error al obtener empresas";
  ...
  const getCompanyBySlug = async (
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error al obtener empresa";
...
export const useAddCompanyForm = () => {
  const { addCompany, loading, error, success, clearError, clearSuccess } =
    useCompanyService();
  ...
  const submitCompanyForm = async (
    ...
    // Obtener labels
    const industryLabel =
      options.industries.find((i) => i.key === formData.industry)?.label ||
      formData.industry;
    const countryLabel =
      options.countries.find((c) => c.key === formData.country)?.label ||
      formData.country;
    const stateLabel =
      options.currentRegions.find((r) => r.key === formData.state)?.label ||
      formData.state;
